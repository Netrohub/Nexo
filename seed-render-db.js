/**
 * Standalone script to seed Render database
 * Run with: node seed-render-db.js
 * 
 * Make sure DATABASE_URL is set in environment or pass it as argument:
 * DATABASE_URL="postgresql://..." node seed-render-db.js
 */
...
async function main() {
  ...
  try {
    ...
    // Create Seller Users
    const sellers = [
      { username: 'seller1', name: 'John Seller', email: 'seller1@nxoland.com' },
      { username: 'seller2', name: 'Sarah Vendor', email: 'seller2@nxoland.com' },
      { username: 'gameseller', name: 'Game Master', email: 'gameseller@nxoland.com' },
    ];
    ...
    for (const seller of sellers) {
      ...
      await prisma.userRole.upsert({
        where: {
          user_id_role_id: { user_id: s.id, role_id: sellerRole.id },
        },
        update: {},
        create: {
          user_id: s.id,
      ...
      await prisma.userRole.upsert({
        where: {
          user_id_role_id: { user_id: s.id, role_id: userRole.id },
        },
        update: {},
        create: {
          user_id: s.id,
    ...
    const users = [
      { username: 'user1', name: 'Test User', email: 'user1@nxoland.com' },
      { username: 'user2', name: 'Another User', email: 'user2@nxoland.com' },
      { username: 'buyer123', name: 'Regular Buyer', email: 'buyer@nxoland.com' },
    ];
    ...
    for (const user of users) {
      ...
      await prisma.userRole.upsert({
        where: {
          user_id_role_id: { user_id: u.id, role_id: userRole.id },
        },
        update: {},
        create: {
          user_id: u.id,
    ...
    console.log('\n✨ All users can access the application with these credentials!');
