...
# NXOLand Backend Deployment Script for Ploi
set -e
...
if [ ! -d "/home/ploi/api.nxoland.com" ]; then
    echo "❌ Error: This script must be run on the Ploi server"
    echo "💡 Upload this script to your server and run it there"
    exit 1
fi
...
if [ ! -f "composer.json" ]; then
    ...
    echo "💡 Make sure you've uploaded all backend files first"
    exit 1
fi
